/**
 * NIFTY options optimizer engine — pricing, generators, multi-factor scoring.
 */

import {
  buildMarketContext,
  scoreCandidateWithFactors,
  ADVANCED_PROFILES,
} from '../lib/optionsMarketContext.js';
...
function evaluateCandidate(legs, refSpot, lotSize) {
  ...
  const payoffCurve = pts.filter((_, i) => i % 2 === 0 || i === pts.length - 1).map((p) => ({ spot: p.spot, pnl: p.value }));
  return {
    legs, maxProfit: Number(maxProfit.toFixed(2)), maxLoss: Number(maxLoss.toFixed(2)), entryCredit, payoffAtSpot,
    breakEvens: [...new Set(be)], profitZones, lossZones, profitZoneWidth, rewardToRisk: rr,
    totalLots: legs.reduce((s, l) => s + l.quantity, 0), payoffCurve,
  };
}
...
const CALENDAR_PROFILES = [
  { key: 'cal-balanced', label: 'Best Calendar Overall', description: 'Best balance across calendar strategies.', score: (c) => ((Math.max(c.entryCredit, 0) + 1) * (c.profitZoneWidth + 1)) / Math.max(Math.abs(c.maxLoss), 1), useAdvanced: false },
  { key: 'cal-hedging', label: 'Calendar Hedging Fit', description: 'VIX-aware calendar structures for theta capture with hedge.', score: (c, ctx) => scoreCandidateWithFactors(c, ctx).compositeScore, useAdvanced: true },
  { key: 'cal-credit', label: 'Calendar Highest Credit', description: 'Largest credit from selling near-term, buying far-term.', score: (c) => Math.max(c.entryCredit, 0) * 1000 + c.profitZoneWidth, useAdvanced: false },
  { key: 'cal-safety', label: 'Calendar Safest', description: 'Lowest max loss with calendar structure.', score: (c) => (c.maxLoss >= 0 ? 1e9 : 1 / Math.abs(c.maxLoss)) * 1000 + Math.max(c.entryCredit, 0), useAdvanced: false },
];

function formatStrategy(c, i, profile, marketContext, lotSize) {
  const factorScores = profile.useAdvanced ? scoreCandidateWithFactors(c, marketContext) : null;
  return {
    rank: i + 1,
    family: c.family,
    legCount: c.legs.length,
    legs: c.legs.map((l) => ({ side: l.side, type: l.type, strike: l.strike, quantity: l.quantity, premium: l.premium })),
    entryCredit: c.entryCredit,
    maxProfit: c.maxProfit,
    maxLoss: c.maxLoss,
    payoffAtSpot: c.payoffAtSpot,
    breakEvens: c.breakEvens,
    profitZones: c.profitZones,
    lossZones: c.lossZones,
    profitZoneWidth: c.profitZoneWidth,
    rewardToRisk: c.rewardToRisk,
    totalLots: c.totalLots,
    payoffCurve: c.payoffCurve || [],
    score: Number((profile.useAdvanced && factorScores ? factorScores.compositeScore * 1000 : profile.score(c, marketContext)).toFixed(2)),
    factorScores: factorScores?.factors || null,
    compositeScore: factorScores?.compositeScore ?? null,
  };
}

function rankProfiles(unique, profiles, marketContext, lotSize, limit = 8) {
  return profiles.map((profile) => {
    const ranked = [...unique].sort((a, b) => {
      const sa = profile.useAdvanced ? scoreCandidateWithFactors(a, marketContext).compositeScore : profile.score(a, marketContext);
      const sb = profile.useAdvanced ? scoreCandidateWithFactors(b, marketContext).compositeScore : profile.score(b, marketContext);
      return sb - sa;
    });
    return {
      key: profile.key,
      ...
      strategies: ranked.slice(0, limit).map((c, i) => formatStrategy(c, i, profile, marketContext, lotSize)),
    };
  });
}
...
export async function runOptimizer(body = {}) {
  ...
  const marketContext = buildMarketContext({
    indiaVix: baseVolatility,
    niftySpot: spot,
    indices,
    expiryDays: Number((timeYears * 365).toFixed(2)),
  });
  ...
  if (isCalendar) {
    ...
    if (!sellExpiry || !buyExpiry) {
      return { error: 'Calendar mode requires sellExpiry and buyExpiry unix timestamps.' };
    }
    ...
    const profiles = rankProfiles(unique, CALENDAR_PROFILES, marketContext, lotSize, 5);
    return {
      mode: 'calendar',
      spot,
      lotSize,
      ...
      indiaVix: baseVolatility,
      marketContext,
      sellExpiryDays: Number((computeTimeYears(sellExpiry) * 365).toFixed(2)),
      buyExpiryDays: Number((computeTimeYears(buyExpiry) * 365).toFixed(2)),
      totalCombinationsEvaluated: candidates.length,
      viableCandidates: unique.length,
      profiles,
    };
  }
  ...
  const allProfiles = [
    ...ADVANCED_PROFILES.map((p) => ({
      ...p,
      score: PROFILES.find((x) => x.key === p.key)?.score || ((c) => scoreCandidateWithFactors(c, marketContext).compositeScore),
    })),
    ...PROFILES.filter((p) => !ADVANCED_PROFILES.some((a) => a.key === p.key)).map((p) => ({ ...p, useAdvanced: false })),
  ];
  const profiles = rankProfiles(unique, allProfiles, marketContext, lotSize, 8);
  ...
  return {
    ...
    familyCounts,
    bestPerFamily: bestPerFamilyList,
    profiles,
  };
}
