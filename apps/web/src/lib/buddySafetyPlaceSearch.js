 ...
 */
export function useGeocodeSearch(query, { minLength = 2, debounceMs = 350, proximity = null } = {}) {
  const [results, setResults] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const nearLat = proximity?.lat;
  const nearLng = proximity?.lng;
  const nearCity = proximity?.city || 'Bengaluru';
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (Number.isFinite(Number(nearLat)) && Number.isFinite(Number(nearLng))) {
          params.set('nearLat', String(nearLat));
          params.set('nearLng', String(nearLng));
        }
        if (nearCity) params.set('nearCity', nearCity);

        const response = await fetch(
          `/api/buddy-safety/geocode?${params}`,
        ...
        if (!next.length) {
          const hint = nearCity
            ? `No places found near ${nearCity}. Try a landmark name, e.g. "Prestige Tech Park".`
            : 'No places found. Try a landmark plus city.';
          setError(hint);
        }
      } catch (err) {
  ...
  }, [query, minLength, debounceMs, nearLat, nearLng, nearCity]);
